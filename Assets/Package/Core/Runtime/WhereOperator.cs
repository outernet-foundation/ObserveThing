...
{
    public class WhereOperator<T> : IDisposable
    {
        ...
        public WhereOperator(ICollectionOperator<T> source, Func<T, IValueOperator<bool>> where, ICollectionObserver<T> receiver)
        {
            _receiver = receiver;
            _where = where;
