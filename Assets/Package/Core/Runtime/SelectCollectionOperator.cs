...
{
    public class SelectCollectionOperator<T, U> : IDisposable
    {
        ...
        public SelectCollectionOperator(ICollectionOperator<T> source, Func<T, U> select, ICollectionObserver<U> receiver)
        {
            _receiver = receiver;
            _select = select;
