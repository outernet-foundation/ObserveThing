...
{
    public class SelectManyOperator<T, U> : IDisposable
    {
        ...
        public SelectManyOperator(ICollectionOperator<T> source, Func<T, ICollectionOperator<U>> select, ICollectionObserver<U> receiver)
        {
            _receiver = receiver;
            _select = select;
