...
namespace ObserveThing
{
    public class CreateCollectionObservable<T, U> : ICollectionObservable<U> where U : IDisposable
    {
        ...
        public CreateCollectionObservable(ICollectionObservable<T> collection, Func<T, U> select)
        {
        ...
        private class Instance : IDisposable
        {
            ...
            public Instance(IObservable source, ICollectionObservable<T> collection, Func<T, U> select, IObserver<CollectionEventArgs<U>> observer)
            {
                ...
                _observer = observer;
                _args.source = source;
                _source = collection.Subscribe(HandleSourceChanged, HandleSourceError, HandleSourceDisposed);
            }
